...
st.markdown("""
    <style>
        ...
        .user-bubble {
            background-color: #1E1F20; color: #FFFFFF;
            padding: 1.2rem; border-radius: 1.2rem;
            margin-bottom: 1.5rem; border: 1px solid #3C4043;
        }
        .agent-bubble {
        ...
        .stTextInput input::placeholder { color: #FFFFFF !important; opacity: 0.8; }
...
# Access is ready only if both name and valid email are present
...
if st.session_state.current_page == "Settings":
    ...
    # User Name Input
    name_input = st.text_input("Your Name", value=st.session_state.user_name, placeholder="Enter your name")
    
    # Parent Email Input
    email_input = st.text_input("Trusted Contact Email", value=st.session_state.parent_email, placeholder="parent@example.com")
...
# ---------------------------------------------------------
    ...
    if not st.session_state.chat_history:
        ...
        st.markdown("<h2 style='color: #C4C7C5; font-weight: 400; margin-top: 0;'>How can I help you today?</h2>", unsafe_allow_html=True)

    for entry in st.session_state.chat_history:
        if entry["role"] == "user":
            st.markdown(f"<div class='user-bubble'><b>{st.session_state.user_name}</b><br>{entry['content']}</div>", unsafe_allow_html=True)
        elif entry["role"] == "agent":
            st.markdown(f"<div class='agent-bubble'>🤖 <b>Agent</b><br>{entry['content']}</div>", unsafe_allow_html=True)
        elif entry["role"] == "emotion":
            st.markdown(f"<span style='color: #8AB4F8; font-size: 0.8rem;'>✨ {entry['content']}</span>", unsafe_allow_html=True)

    st.markdown("</div>", unsafe_allow_html=True)

    # 💬 BOTTOM INPUT
    st.markdown("<div style='position: fixed; bottom: 0; left: 0; width: 100%; background: #0E0E10; padding: 20px 0; z-index: 999;'>", unsafe_allow_html=True)
    ...
    with center_col:
        ...
        with st.form("chat_form", clear_on_submit=True):
            ...
            user_input = input_cols[0].text_input("", placeholder="Enter here ...", label_visibility="collapsed", disabled=not access_ready)
        ...
        if submit and user_input and access_ready:
            ...
            st.session_state.chat_history.append({"role": "emotion", "content": f"{emotion} ({int(score*100)}%)"})
            st.session_state.chat_history.append({"role": "agent", "content": agent_reply})

            if emotion.lower() in ["sadness", "fear", "anger"] and score > 0.8:
                # Alert now includes the User Name correctly
                alert_msg = (
